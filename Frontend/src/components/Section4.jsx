...
const {t}=useTranslation()
  ...
  return (
    <div className='bg-primary pt-7'>
        ...
        <h1 className='font-dancing sm:text-[36px] text-[20px] text-gray-900'><span className='sm:text-[64px] text-[40px]'>{t("Summer")}</span> {t("Collections")}</h1> 
        <p className='font-playfair text-[20px] text-gray-900'>{t("We consider  your look and comfort on scorching weather.")}</p>
        </div>
    <div className='w-full h-auto flex flex-col gap-8 md:gap-0 md:flex-row justify-around items-center'>
      <div className='md:w-[45vw] md:h-[80vh] w-full h-auto flex md:flex-col flex-col-reverse gap-8 items-center justify-start'>
        ...
        <div className='text-center lg:w-[30vw] md:w-[35vw] w-[90vw]'>
            <p className='font-playfair'>{t("Our main aim is to serve our customer with better quality product. We try to understand their needs and provide them within a short period of time.")}</p>
        </div>
      </div>
      <div className='md:w-[45vw] flex flex-col items-center justify-center pt-4'>
         <div className='md:w-[40vw] w-[90vw] pb-8'>
            <p className='font-playfair'>{t("We provide the largest clothing collection for any season. You can choose trendy or classy design according to your preferences. Our services are super fast and we update within 24 hours.")}</p>
         </div>
         <div>
         ...
         <a href="" className='font-playfair bg-tertiary border-[2px] border-quaternary px-8 py-4 rounded-2xl relative lg:bottom-36 md:bottom-28 bottom-24'>{t("Explore")}</a>
         </div>
